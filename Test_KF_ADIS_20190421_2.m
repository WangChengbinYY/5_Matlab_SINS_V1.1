...
%% 2.1 全局参数
    ...
    [L,m] = size(Data_IMU_R);
%% 2.2 起点信息
    G_Start_Att = [-15.2559*pi/180;-5.8320*pi/180;-90*pi/180];
    G_Start_Vel = [0;0;0];
...
%% 2.3 各类控制参数设置
    ...
    %L = 80*1000;
    %(2)KF模型参数
    KF_switch = 0;  %1 进行KF滤波，0 不进行KF滤波
...
%     INSData_Now.Q_b_n = Att_DCM2Q(INSData_Now.C_b_n);
    ...
    %(4)保存导航结果
        ...
        %TEST
%         Result_AVP(i,5:7) = G_Start_Vel;
%         Result_AVP(i,8:10) = G_Start_Pos;
    %(5)更新KF滤波所需的参数
    if KF_switch == 1      
