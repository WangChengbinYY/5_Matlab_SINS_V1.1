...
% 1. 获取原始数据
    clear;clc;
    load('D:\ADI_L_R_B_3_静止1_60000_240000.mat')
...
% 2. 初始参数设定    
    ...
    % 陀螺零偏赋初值
    gyroBias = mean(gyroData(1:3*Fs,:));          %  去前3s数据平均，作为陀螺的 初始零偏
...
% 4. KF滤波参数初始化
    % 初始方差阵P0 6维 姿态误差  零偏误差     姿态 零偏  都按照 XYZ 顺序排列
        ...
        tmp_VarBias = [10,10,10].*(pi/180/3600);
    ...
    % 系统状态噪声方差阵
        ...
        varGyroNoise = ([110,110,110].*(pi/180/3600)).^2;
        varGyroDriftNoise =  ([10,10,10].*(pi/180/3600)).^2;
        Qw = diag([varGyroNoise.*(periodSensor^2),varGyroDriftNoise.*(periodSensor^2)]);
        %Qw = diag([varGyroNoise,varGyroDriftNoise]);
        %Qw = diag([deg2rad([0.1,0.1,0.1]),varGyroDriftNoise]);

    % 系统测量噪声方差阵 
        % 这次实验采用 仿真 姿态观测量来进行  按照KF的更新周期，精度按照 0.1度 来实验
        varZ = 0.1;    % Z 的姿态观测精度  单位 度
        varDB = 10*log10(deg2rad(varZ)^2);
    % 仿真观测数据    
...
tpKk=recordKk(:,:,1);
   % 绘制结果
        % 零偏估计比较
        figure;
         plot(recordGyroBias(:,2).*(180/pi*3600),'r');
        legend('估计零偏'); title('X轴零偏'); ylabel('度/h');        
        figure;
        plot(recordGyroBias(:,1).*(180/pi*3600),'r');
        legend('估计零偏'); title('Y轴零偏'); ylabel('度/h');   
        figure;
        hold on; plot(recordGyroBias(:,3).*(-180/pi*3600),'r');
        legend('估计零偏'); title('Z轴零偏'); ylabel('度/h');
        
	% 姿态 结果
        figure;
        plot(rad2deg( recordOrient(:,3)));
        title('姿态误差 X 横滚'); ylabel('度');
        figure;
        plot(rad2deg( recordOrient(:,2)));
        title('姿态误差 Y 横滚'); ylabel('度');    
        figure;
        plot(rad2deg( recordOrient(:,1)));
