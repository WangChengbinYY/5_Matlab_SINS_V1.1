...
%                 Z = rotvec(Z_q)';
   ...
   % 绘制结果
	...
	% 姿态 结果
        time = [1:L]/Fs;
        timeZ = [1:L_Z]./(Fs/Fs_KF);
        recordOrient_deg = rad2deg(recordOrient);
        AttitudeTrue_deg = rad2deg(AttitudeTrue);        
        figure;
        plot(timeZ,AttitudeTrue_deg(:,1) ,'r');
        hold on;  plot(time,recordOrient_deg(:,3),'b-*');        
        title('姿态估计 X 横滚'); ylabel('度');
        figure;
        plot(timeZ,AttitudeTrue_deg(:,2) ,'r');
        hold on;  plot(time,recordOrient_deg(:,2),'b-*');     
        title('姿态估计 Y 横滚'); ylabel('度');
        figure;
        plot(timeZ,AttitudeTrue_deg(:,3) ,'r');
        hold on;  plot(time,recordOrient_deg(:,1),'b-*');     
        title('姿态估计 Z 横滚'); ylabel('度');     
