...
% 4. 起始姿态、速度、位置设定
G_Start_Att(1,1) = 9.5e-4;   %姿态 俯仰角 度
G_Start_Att(2,1) = -6e-3;   %姿态 横滚角 度
G_Start_Att(3,1) = 10.0 * G_Const.D2R;   %姿态 航向角 (北偏西为正！！)
...
for i=2:n
    ...
    INSData_Now.w_ib_b = Data_IMU_R(i,5:7)'-Bias_Gyro;           
...
Plot_AVP_Group(Result_AVP,Result_AVP_NoBias);
% 
% %% 三、MIMU惯导解算
% % 1. 数据准备
% [n,m] = size(Data_IMU_R);
